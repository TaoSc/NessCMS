<?php
	...
	class Handling {
		static function getNews($condition = '0 = 0', $visible = true, $languageCheck = true, $offsetLimit = false) {
			global $db;
			if ($offsetLimit)
				$offsetLimit = ' LIMIT ' . $offsetLimit;

			$request = $db->query('SELECT id FROM posts WHERE ' . $condition . ' ORDER BY id DESC' . $offsetLimit);
